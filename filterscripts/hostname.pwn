...
new hostname[][128] =
{
    {"hostname Х ARIZONA TEST | (feature/fix bugs/obnova)"},
    {"hostname Х ARIZONA TEST | (feature/fix bugs/obnova)"},
    {"hostname Х ARIZONA TEST | (feature/fix bugs/obnova)"},
    {"hostname Х ARIZONA TEST | (feature/fix bugs/obnova)"}
}, Owner_NickNames[][24] =
{
    "Churikov"
}, MySQL:LOG_BASE, bool:WRITE_ACTION_SERVER = false, global_str[144];
...
{
    for(new i; i < sizeof(Owner_NickNames); i++)
        if(!strcmp(name, Owner_NickNames[i], false))
            return true;
